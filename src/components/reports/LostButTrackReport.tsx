...
import { OPPORTUNITY_STATUS_LABELS } from '../../types/OpportunityStatusChoices.ts';

interface Props {
  isDashboard?: boolean;
}

export const LostButTrackReport: React.FunctionComponent<Props> = (props) => {
  ...
  useEffect(() => {
    if (isDashboard) {
      authFetchJson(`${API_BASE_URL}/dashboard/lost-but-tracked`)
        .then(({ json }) => {
          setRows(json);
          setLoading(false);
        })
      ...
      authFetchJson(`${API_BASE_URL}/reports/lost-but-tracked`)
        .then(({ json }) => {
          setRows(json);
          setLoading(false);
        })
  ...
  const exportToCSV = () => {
    const headers = ['Closed', 'Follow Up Date', 'Status', 'Contact', 'Opportunity'];
    const csvRows = rows.map((row) => [
      new Date(row.dateClosed ?? '').toLocaleDateString(),
    ...
    a.href = url;
    a.download = `lost-but-tracked-report-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
